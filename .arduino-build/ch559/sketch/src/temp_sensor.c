...
void TempController_begin(TempController* tc, DS18B20* sensor, VoltageController* vc) {
    ...
    DS18B20_setResolution(tc->tempSensor, DS18B20_RESOLUTION_10BIT);
    
    // 设置报警范围
    DS18B20_setAlarmTemperature(tc->tempSensor, TEMP_MAX, TEMP_MIN);
...
bool TempController_update(TempController* tc) {
    ...
    target_voltage = TempController_mapTempToVoltageLinear(temp);
#else
    target_voltage = TempController_mapTempToVoltageQuadratic(temp);
#endif

    VoltageController_setVoltage(tc->voltCtrl, target_voltage);

    tc->target_rpm = TempController_mapTempToRPM(temp);
